...
src2D = CasingSimulations.Sources.DownHoleCasingSrc(mesh2D, cp)


# In[16]:

src3D = CasingSimulations.Sources.DownHoleCasingSrc(mesh3D, cp)
...
np.save('fields2DMultiFreqDownhole', fields2D[:, 'hSolution'])
...
np.save('fields2DMultiFreqDownhole', fields3D[:, 'hSolution'])
